package com.vivid.akka.simpleStart;
...
public class RouterTest extends UntypedActor {
...

	{
		...
		/**
		 * RoundRobinRoutingLogic: 轮询 BroadcastRoutingLogic: 广播 RandomRoutingLogic: 随机
	...
	public void onReceive(Object o) throws Throwable {
		if (o instanceof InboxTest.Msg) {
			...
			System.out.println(
					((Terminated) o).actor().path() + " 该actor已经删除。router.size=" + router.routees().toVector().size());

			if (router.routees().toVector().size() == 0) {// 没有可用actor了
